from myhdl import block, concat, intbv, Signal, always_comb, always, instances
...
def pipeline(clock, ext_interrupts, reset, imem_wait, imem_addr, imem_rdata, imem_badmem_e, dmem_wait, dmem_en,
    ...
    def load_data(address, data, mem_type):
        ...
        b_extend = intbv(0)[32:0]
        if shifted_data[7]:
            b_extend = intbv(((1 << 24) - 1) << 8)[32:0]

        h_extend = intbv(0)[32:0]
        if shifted_data[15]:
            h_extend = intbv(((1 << 16) - 1) << 16)[32:0]
    ...
    PC_src_sel = Signal(intbv(0)[PC_SRC_SEL_WIDTH:])
    PC_PIF = Signal(intbv(0)[XPR_LEN:])

    PC_IF = Signal(intbv(0)[XPR_LEN:])

    kill_IF = Signal(False)
    stall_IF = Signal(False)

    PC_DX = Signal(intbv(0)[XPR_LEN:])
    inst_DX = Signal(intbv(0)[INST_WIDTH:])

    kill_DX = Signal(False)
    stall_DX = Signal(False)

    imm_type = Signal(intbv(0)[IMM_TYPE_WIDTH:])
    imm = Signal(intbv(0)[XPR_LEN:])
    src_a_sel = Signal(intbv(0)[SRC_A_SEL_WIDTH:])
    src_b_sel = Signal(intbv(0)[SRC_B_SEL_WIDTH:])
    rs1_addr = Signal(intbv(0)[REG_ADDR_WIDTH:])
    rs1_data = Signal(intbv(0)[XPR_LEN:])
    rs1_data_bypassed = Signal(intbv(0)[XPR_LEN:])
    rs2_addr = Signal(intbv(0)[REG_ADDR_WIDTH:])
    rs2_data = Signal(intbv(0)[XPR_LEN:])
    rs2_data_bypassed = Signal(intbv(0)[XPR_LEN:])
    alu_op = Signal(intbv(0)[ALU_OP_WIDTH:])
    alu_src_a = Signal(intbv(0)[XPR_LEN:])
    alu_src_b = Signal(intbv(0)[XPR_LEN:])
    alu_out = Signal(intbv(0)[XPR_LEN:])
    cmp_true = Signal(False)
    bypass_rs1 = Signal(False)
    bypass_rs2 = Signal(False)
    dmem_type = Signal(intbv(0)[MEM_TYPE_WIDTH:])

    md_req_valid = Signal(False)
    md_req_ready = Signal(False)
    md_req_in_1_signed = Signal(False)
    md_req_in_2_signed = Signal(False)
    md_req_out_sel = Signal(intbv(0)[MD_OUT_SEL_WIDTH:])
    md_req_op = Signal(intbv(0)[MD_OP_WIDTH:])
    md_resp_valid = Signal(False)
    md_resp_result = Signal(intbv(0)[XPR_LEN:])

    PC_WB = Signal(intbv(0)[XPR_LEN:])
    alu_out_WB = Signal(intbv(0)[XPR_LEN:])
    csr_rdata_WB = Signal(intbv(0)[XPR_LEN:])
    store_data_WB = Signal(intbv(0)[XPR_LEN:])

    kill_WB = Signal(False)
    stall_WB = Signal(False)
    bypass_data_WB = Signal(intbv(0)[XPR_LEN:])
    load_data_WB = Signal(intbv(0)[XPR_LEN:])
    wb_data_WB = Signal(intbv(0)[XPR_LEN:])
    reg_to_wr_WB = Signal(intbv(0)[REG_ADDR_WIDTH:])
    wr_reg_WB = Signal(False)
    wb_src_sel_WB = Signal(intbv(0)[WB_SRC_SEL_WIDTH:])
    dmem_type_WB = Signal(intbv(0)[MEM_TYPE_WIDTH:])

    # CSR Management
    csr_addr = Signal(intbv(0)[CSR_ADDR_WIDTH:])
    csr_cmd = Signal(intbv(0)[CSR_CMD_WIDTH:])
    csr_imm_sel = Signal(False)
    prv = Signal(intbv(0)[PRV_WIDTH:])
    illegal_csr_access = Signal(False)
    interrupt_pending = Signal(False)
    interrupt_taken = Signal(False)
    csr_wdata = Signal(intbv(0)[XPR_LEN:])
    csr_rdata = Signal(intbv(0)[XPR_LEN:])
    retire_WB = Signal(False)
    exception_WB = Signal(False)
    exception_code_WB = Signal(intbv(0)[ECODE_WIDTH:])
    handler_PC = Signal(intbv(0)[XPR_LEN:])
    eret = Signal(False)
